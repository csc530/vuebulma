<template>
  ...
  <bulma-box tag="section">
    ...
    <bulma-dropdown :content="dropdownList" alignment="right">
      <button>button trigger</button>
      <!--			todo: make it less clunky to button dropwdown class to parent element-->
      <template v-slot:dropdown-item="data">
        <a href="#" v-bind:class="data.class">{{ data.data }}</a>
      </template>
    </bulma-dropdown>
    <BulmaDropdown :content="[1,2,3,4,3]" is-hoverable tag="p">
      <p class="button">Hoverable</p>
    </BulmaDropdown>
    <bulma-dropdown :content="[-1,-2,-3,-443,-5,-32,-123-23,-2,-2,-912.34]" is-hoverable="" on-click="false">
...
//! Dropdown
const dropdownList = ['look', 'I', 'trigger', 'on', 'click'];
